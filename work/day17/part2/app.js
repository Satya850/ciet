...
app.post("/login", async function(req, res){
    const era = await Country({ date: req.body.date });
        const hello = await Country({ address: req.body.address });
    try {
        // check if the user exists
        const user = await Country.findOne({ username: req.body.username }); 
        if (user) {
